...
def _derive_key() -> bytes:
    if not settings.encryption_key:
        raise RuntimeError("ENCRYPTION_KEY not set. Generate one via: python -c \"from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())\"")
    password = settings.encryption_key.encode()
    salt = settings.encryption_salt.encode() if settings.encryption_salt else b"ma-dd-ai-salt"
